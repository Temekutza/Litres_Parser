...
- **Нормализация**: данные нормализуются **ПРИ ПАРСИНГЕ** (даты → DD.MM.YYYY, цены → `569,00`, возраст → `18`, и т.д.)
- **Прогресс** сохраняется в `litres.sqlite` — можно запускать повторно.

### Архитектура

```
Парсинг → Сырой JSON → Нормализация → Нормализованный JSON → База данных → Excel
```

Все данные в базе уже нормализованы и готовы к экспорту.
